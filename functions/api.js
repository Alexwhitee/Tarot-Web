export async function onRequestPost({ request }) {
    const { text, pms, spread } = await request.json();

    // 构建排阵信息
    let spreadInfo = "";
    if (spread) {
        spreadInfo = `
你将使用"${spread.name}"排阵进行解读，这种排阵有${spread.cardCount}个位置，分别是：${spread.positions.join('、')}。
每个位置的含义如下：
${spread.positions.map((pos, idx) => `${idx + 1}. ${pos}`).join('\n')}

请根据每张牌在排阵中的位置进行详细解读，并在最后给出综合分析。
`;
    }
...
${spreadInfo}`
            ...
            {
                "role": "user",
                "content": `卡牌数组是：${JSON.stringify(pms)}，问题是：'${text}？'，请根据${spread ? spread.name : "三张牌占卜"}排阵帮我解析`
            }
        ],
        "stream": false,
        "model": "glm-4-flash",
